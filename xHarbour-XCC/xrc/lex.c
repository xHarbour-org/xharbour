 ...
 * File    : lex.c                                                          *
 *                                                                          *
 * Purpose : Win32 Resource Compiler; preprocessor; input lexer.            *
 *                                                                          *
 * History : Date      Reason                                               *
 *           97-10-14  Created                                              *
 *                                                                          *
...
struct fsm
{
    ...
    uchar_t ch[5];      /* and see one of these characters */
...
void setup_lexfsm(void)
{
    ...
    /* special cases for ? (trigraphs), \ (splicing), runes, and EOB */
...
reswitch:
            switch (state & 0x7F)
            {
                ...
                case S_EOF:
                    ...
                    tp->len = 0;
                    s->inp = ip;
                    if (tp != trp->bp && (tp-1)->type != NL && cursource->hf != NULL)
...
#ifndef STDIO
    if (s->inp[1] == '\r' && s->inp[2] == '\n')
    {
        memmove(s->inp, s->inp+3, s->inl-s->inp+4);
        s->inl -= 3;
        return TRUE;
    }
#endif
...
static int fillbuf(SOURCE *s)
{
    ...
    if (s->hf == NULL || !ReadFile(s->hf, s->inl, INS/8, &n, NULL))
        n = 0;
 ...
 *                                                                          *
 *           If hf != NULL and str == NULL, then from a file 'name';        *
 *           if hf == NULL and str, then from the string.                   *
 *                                                                          *
 * History : Date      Reason                                               *
 *           97-10-14  Created                                              *
 *                                                                          *
...
SOURCE *set_source(const char *filename, HANDLE hf, const char *str)
{
    SOURCE *s = new(SOURCE);
    int len;
