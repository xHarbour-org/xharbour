 ...
 * File    : cpp.h                                                          *
 *                                                                          *
 * Purpose : ISO C Compiler; Preprocessor; Constants and definitions.       *
 *                                                                          *
 * History : Date      Reason                                               *
 *           00-06-06  Created                                              *
 *           03-06-26  New codes for EOB and EOFC (avoid using high bit).   *
 *           04-11-24  Added MSCOM to handle special /##/ pasting.          *
 *                                                                          *
 ****************************************************************************/
...
enum kwtype {
    ...
    KLINENO, KFILE, KDATE, KTIME, KSTDC, KSTDCHOSTED, KSTDCVERSION,
    KSTDCIEC559, KSTDCISO10646, KEVAL
};
...
#define ISVARIADIC  16      /* has variadic argument */
...
SOURCE *set_source(const char *, int, const char *);
void unset_source(void);

/* nlist.c */
void setup_kwtab(bool_t);
...
void define_or_undefine(TOKENROW *, int);
void expand_tokenrow(TOKENROW *, const char *);

/* eval.c */
intmax_t eval(TOKENROW *, int);
