 ...
 * File    : export.c                                                       *
 *                                                                          *
 * Purpose : Win32 Library Manager; export management.                      *
 *                                                                          *
 * History : Date      Reason                                               *
 *           98-01-18  Created                                              *
 *           98-11-07  Support for new import archives added.               *
 *           99-03-27  Use same prefix for code and data: "__imp_".         *
 *           04-01-12  Added process_export_list() and renamed to export.c  *
 *                                                                          *
 ****************************************************************************/
...
static size_t write_old_import_member(void *, char *, bool_t, int);
static size_t write_new_import_member(void *, char *, bool_t, int, bool_t, char *);
 ...
 *           04-01-13  Bugfix: index address table through ordinal table.   *
 *           04-01-13  Bugfix: walk through name table, not address table.  *
 *           04-01-13  Handle export without name (use ordinal).            *
 *                                                                          *
 ****************************************************************************/
...
void process_executable_image(FILEINFO *exe_file)
{
     ...
     */
    for (i = 0; i < export->e_nnames; i++)
    {
        char decname[MAX_PATH], *name;
        bool_t is_func;
        long fwdptr;

        name = nameptr_table[i] + (char *)base;
        sh = get_section_header_by_address(address_table[ordinal_table[i]], pehdrs);

        /* Assume function, if located in code segment... */
        is_func = (sh->s_flags & COFF_STYP_TEXT);

        fwdptr = sh->s_scnptr + (address_table[ordinal_table[i]] - sh->s_vaddr);
        ...
        if (options.old_implib)
        {
            ...
            process_object_module(member_name, write_old_import_member(
                bp, name, is_func, 0), bp, time_stamp);
        }
        else
        {
            /* Write import object in the new, terse, format */
            process_object_module(member_name, write_new_import_member(
                bp, name, is_func, 0, name == decname, member_name), bp, time_stamp);
        }
    }
    for (; i < export->e_naddrs; i++)
    {
        if (ordinal_table[i] >= export->e_nnames &&
            ordinal_table[i] < export->e_naddrs)
        {
            /* export without name, i.e. use ordinal */
            int ordinal = ordinal_table[i] + export->e_ordbase;

            if (options.old_implib)
            {
                /* Write import object in the old, verbose, format */
                process_object_module(member_name, write_old_import_member(
                    bp, "noname", TRUE, ordinal), bp, time_stamp);
            }
            else
            {
                /* Write import object in the new, terse, format */
                process_object_module(member_name, write_new_import_member(
                    bp, "noname", TRUE, ordinal, FALSE, member_name), bp, time_stamp);
            }
        }
    }
...
/****************************************************************************
 *                                                                          *
 * Function: process_export_list                                            *
 *                                                                          *
 * Purpose : Process all command line exports (/DEF or /EXPORT).            *
 *                                                                          *
 * History : Date      Reason                                               *
 *           04-01-12  Created                                              *
 *                                                                          *
...
void process_export_list(void)
{
    ...
    /*
     * Set default machine, if missing.
     */
    if (options.machine == MACHINE_UNKNOWN)
    {
        apperror(RCWARNING(ERROR_NO_MACHINE));
        options.machine = MACHINE_X86;
    }

    build_special_symbols(export_fname);
     ...
     * Write the special import archive members.
     */
    process_object_module(export_fname, write_import1_member(bp, export_fname), bp, time_stamp);
    process_object_module(export_fname, write_import2_member(bp), bp, time_stamp);
    process_object_module(export_fname, write_import3_member(bp), bp, time_stamp);
    ...
    for (i = 0; i < export_count; i++)
    {
        ...
        {
            /* Write import object in the old, verbose, format */
            process_object_module(export_fname, write_old_import_member(
                bp, name, exp->is_func, exp->ordinal), bp, time_stamp);
        }
        else
        {
            /* Write import object in the new, terse, format */
            process_object_module(export_fname, write_new_import_member(
                bp, name, exp->is_func, exp->ordinal, analyze, export_fname), bp, time_stamp);
        }
    }
 ...
 * History : Date      Reason                                               *
 *           98-01-18  Created                                              *
 *           04-01-12  Support for ARM machine added.                       *
 *                                                                          *
 ****************************************************************************/
...
static size_t write_import1_member(void *ip, const char *member_name)
{
    ...
    ohdr->o_stack_reserve = DEFAULT_STACK_RESERVE;  /* see MS */
    ohdr->o_stack_commit = DEFAULT_STACK_COMMIT;  /* see MS */
    ohdr->o_heap_reserve = DEFAULT_HEAP_RESERVE;  /* see MS */
    ohdr->o_heap_commit = DEFAULT_HEAP_COMMIT;  /* see MS */
...
static size_t write_old_import_member(void *ip, char *expname, bool_t is_func, int ordinal)
{
    COFF_FILHDR *fhdr;
    COFF_SCNHDR *sh;
    COFF_RELOC *re;
 ...
 *           03-10-04  Bugfix: Bad initialization of flags (cast?).         *
 *           04-01-12  Support for ARM machine added.                       *
 *           04-01-12  Support for ordinals added.                          *
 *           04-01-13  Support for decorated names added (for /DEF).        *
 *                                                                          *
 ****************************************************************************/

static size_t write_new_import_member(void *ip, char *expname, bool_t is_func, int ordinal, 
    bool_t analyze, char *member_name)
{
    COFF_NEWIMP *ni;
    void *base;
    ...
    ni->ni_ordinal = ordinal;
    ni->ni_flags = (is_func) ? COFF_I_CODE : COFF_I_DATA;
    if (ordinal == 0)
    {
        if (analyze && *expname == '_')
            ni->ni_flags |= (strchr(expname, '@') != 0) ? COFF_I_NAME_UNDEC : COFF_I_NAME_NOPREF;
        else
            ni->ni_flags |= COFF_I_NAME;
    }
