 ...
 * File    : xxscalbn.h                                                     *
 *                                                                          *
 * Purpose : Common scalbn[fl] functionality.                               *
 *                                                                          *
 * History : Date      Reason                                               *
 *           01-08-30  Created                                              *
...
/* scale x by 2^xexp */
FTYPE __cdecl FFUN(scalbn)(FTYPE x, int xexp)
{
...
#ifdef _USE_MATHERR
                {
                    struct _exception e = { _UNDERFLOW, FERR(scalbn) };
...
#ifdef _USE_MATHERR
                {
                    struct _exception e = { _OVERFLOW, FERR(scalbn) };
                    if (_matherr(&e)) return e.retval;
                }
#endif
