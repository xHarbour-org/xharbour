...
/* get a floating point value for __wscanf */
int __wgetflt(__wscninfo *px)
{
        ...
        else
        {
            ...
            *p++ = L'n', ch = WGETN(px);
            ...
            {       /* parse rest of "infinity" */
                if ((ch = WGETN(px)) != L'n' && ch != L'N' || (ch = WGETN(px)) != L'i' && ch != L'I' ||
                    (ch = WGETN(px)) != L't' && ch != L'T' || (ch = WGETN(px)) != L'y' && ch != L'Y')
                    WUNGETN(px, ch);
                else
                {
    ...
    if (dlen > 0)
    {
        ...
        for (nsig = 0; ch != WEOF && wmemchr(&digits[0], ch, dlen); ch = WGETN(px), seen = 1)
        {
        ...
        if (__wctob(ch) == localeconv()->decimal_point[0])
            *p++ = ch, ch = WGETN(px);
        ...
        for (; ch != WEOF && wmemchr(&digits[0], ch, dlen); ch = WGETN(px), seen = 1)
            if (nsig < _MAX_SIG_DIG) *p++ = ch, ++nsig;

        if (seen && (dlen == 10 && (ch == L'e' || ch == L'E') || dlen != 10 && (ch == L'p' || ch == L'P')))
        {
            /* parse exponent */
            *p++ = ch, ch = WGETN(px);
            ...
            for (nsig = 0; iswdigit(ch); ch = WGETN(px), seen = 1)
                    if (nsig < _MAX_EXP_DIG) *p++ = ch, ++nsig;
        }
        WUNGETN(px, ch);
    }
    ...
    *p = L'\0';
    if (!px->noconv)
    {
        ...
        if (dlen <= 10)
        {
            ldval = __wcstold(ac, 0, pten);
        }
        else
        {
            /* convert fraction and scale by hex exponent */
            ldval = __wcstold(ac, 0, 0);
            __fpscalel(&ldval, pten * 4);
        }
