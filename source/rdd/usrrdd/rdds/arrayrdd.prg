...
STATIC FUNCTION AR_INIT( nRDD )
   ...
   /* Init DBF Hash */
	...
	USRRDD_RDDDATA( nRDD, Hash() ) // Modified by Marcio
...
STATIC FUNCTION AR_SKIPRAW( nWA, nRecords )
   ...
   LOCAL nIni := 0
	LOCAL aRecords := aWAData[ WADATA_DATABASE, DATABASE_RECORDS ]  // Addeby by Marcio
   ...
   ELSEIF nIndex > 0
      ...
      ELSEIF nRecords > 0 .AND. nRec + nRecords > nEnd
         nResult := AR_GOTO( nWA, 0 )
         aWAData[ WADATA_ORDRECNO ] := Len( aRecords ) // Modified by Marcio
      ELSE
...
FUNCTION ARRAYRDD_GETFUNCTABLE( pFuncCount, pFuncTable, pSuperTable, nRddID, pSuperRddID )
   ...
   aMyFunc[ UR_RECNO        ] := ( @AR_RECNO()        )	// Addedy by Marcio
...
STATIC FUNCTION Seek( xSeek, lSoft, lLast, aIndexInfo, nRec )
   ...
   CASE 1   /* Archive with 1 record */
      ...
      ENDIF
      EXIT
   DEFAULT // OTHERWISE   /* Archive with 2 or more records */
