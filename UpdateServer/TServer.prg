...
METHOD SendAvailable( cPassword, cFolder ) CLASS TUpdateServer
   ...
   nAt := At( ',', cPassword )
   IF nAt > 0
      cUser := Left( cPassword, nAt - 1 )
      cPassword := SubStr( cPassword, nAt + 1 )
   ELSE
      cUser := ""
   ENDIF

   IF ! ValidateSubscription( cUser, cPassword )
      ::Send( "Authentication failed: " + cPassword )
      RETURN Self
   ENDIF
...
FUNCTION ValidateSubscription( cUser, cPasw )

   LOCAL nCnn, cConnection, cEdition, dEOS
   LOCAL nPresetArea
   LOCAL lPresetCentury
   LOCAL cPresetDateFormat
   LOCAL lRet := .F.

   TRY
      cConnection := "HST=mysql.bekz.be;UID=xHarbour;PWD=xHarbourpas;DTB=xharbourv2"
      nCnn := SR_AddConnection( 3, cConnection)
   CATCH oErr
      TraceLog( "Connection error! ", oErr:Operation, oErr:Description )
      RETURN .F.
   END
   ...
   USE Users NEW VIA "SQLRDD" CONNECTION nCnn
   SET INDEX TO EMAIL

   IF ! USERS->( dbSeek( Lower( cUser ) ) )
      TraceLog( "User not found: " + cUser )
   ...
   IF ! ( Alltrim( Lower( USERS->Password ) ) == Alltrim( Lower( cPasw ) ) )
      TraceLog( "Wrong password: '" + cPasw + "' for user: '" + cUser + "'" )
   ...
   USE Order_Detail NEW VIA "SQLRDD" CONNECTION nCnn ALIAS ORDER
   SET FILTER TO ORDER->Customer_ID = USERS->(RecNo()) .AND. ORDER->ART_Type > 0 .AND. ORDER->ART_Type < 5

   lPresetCentury := Set( _SET_CENTURY )
   cPresetDateFormat := Set( _SET_DATEFORMAT )
   ...
   WHILE ! ORDER->( Eof() )
      IF Empty( dEOS ) .OR. CtoD( Right( Rtrim( ORDER->Sub_End ), 2 ) + "/" + SubStr( ORDER->Sub_End, 5, 2 ) + "/" + Left( ORDER->Sub_End, 4 ) ) < dEOS
         dEOS := CtoD( Right( Rtrim( ORDER->Sub_End ), 2 ) + "/" + SubStr( ORDER->Sub_End, 5, 2 ) + "/" + Left( ORDER->Sub_End, 4 ) )
      ENDIF
   ...
   IF ! lRet
      TraceLog( "Subscription expired on: '" + dToc( dEOS ) + "' for user: '" + cUser + "'" )
   ENDIF

   // Restore Environment.
   SELECT nPresetArea
   Set( _SET_CENTURY, lPresetCentury )
   Set( _SET_DATEFORMAT, cPresetDateFormat )
