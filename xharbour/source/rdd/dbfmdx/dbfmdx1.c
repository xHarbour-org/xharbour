/*
 * $Id: dbfmdx1.c,v 1.63 2009/05/01 21:10:19 marchuet Exp $
 */
...
static HB_ERRCODE hb_cdxOrderCreate( CDXAREAP pArea, LPDBORDERCREATEINFO pOrderInfo )
{
   ...
   if( !fOpenedIndex )
   {
      ...
      do
      {
         if( fTemporary )
         {
            pFile = hb_fileCreateTemp( NULL, NULL, FC_TEMPORARY, ( BYTE * ) szTempFile );
            fNewFile = TRUE;
         }
         else
         {
...
HB_CALL_ON_STARTUP_END( _hb_dbfmdx_rdd_init_ )
   ...
   static HB_$INITSYM hb_vm_auto_dbfmdx1__InitSymbols = dbfmdx1__InitSymbols;
   static HB_$INITSYM hb_vm_auto_dbfmdx_rdd_init = _hb_dbfmdx_rdd_init_;
   #pragma data_seg()
#endif
