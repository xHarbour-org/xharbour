...
static HB_LONG hb_dbfGetRowVer( DBFAREAP pArea, USHORT uiField, HB_LONG * pValue )
{
   ...
   *pValue = 0;
   ...
   if( hb_fileNetReadAt( pArea->pDataFile, ( BYTE * ) &dbField, sizeof( dbField ),
       ...
       sizeof( dbField ) )
   {
      *pValue = HB_GET_LE_UINT64( dbField.bReserved2 ) + 1;
      ...
      hb_fileNetWriteAt( pArea->pDataFile, ( BYTE * ) &dbField, sizeof( dbField ),
                      sizeof( DBFHEADER ) + uiField * sizeof( DBFFIELD ) );
   }
...
static void hb_dbfUpdatedbaselockValue( DBFAREAP pArea, ULONG ulRecNo )
{
   if( pArea->uidbaselock && ulRecNo )
   {
                      ...
                      ( HB_FOFFSET ) pArea->uiRecordLen ) == ( ULONG ) pArea->uiRecordLen )
      {
         ...
         hb_fileNetWriteAt( pArea->pDataFile, pPtr, pArea->lpFields[ pArea->uidbaselock ].uiLen - 3,
                         ( HB_FOFFSET ) pArea->uiHeaderLen +
...
static HB_ERRCODE hb_dbfClose( DBFAREAP pArea )
{
   ...
   if( pArea->pDataFile )
   {
      ...
      if( pArea->fTemporary )
         hb_fileNetDelete( ( BYTE * ) pArea->szDataFileName, uiRemote );
   }
   ...
   if( pArea->fHasMemo && pArea->pMemoFile )
   {
      ...
      if( pArea->fTemporary )
         hb_fileNetDelete( ( BYTE * ) pArea->szMemoFileName, uiRemote );
   }
...
static HB_ERRCODE hb_dbfReadDBHeader( DBFAREAP pArea )
{
   ...
   do
   {
      ...
      if( hb_fileNetReadAt( pArea->pDataFile, ( BYTE * ) &pArea->dbfHeader,
...
static HB_ERRCODE hb_dbfWriteDBHeader( DBFAREAP pArea )
{
   ...
   else
   {
      ...
      {
         hb_fileNetWriteAt( pArea->pDataFile, ( BYTE * ) "\032", 1, llOffset );
   ...
   if( hb_fileNetWriteAt( pArea->pDataFile, ( BYTE * ) &pArea->dbfHeader,
                          sizeof( DBFHEADER ), 0 ) == sizeof( DBFHEADER ) )
   {
      errCode = HB_SUCCESS;
   }
...
static HB_ERRCODE hb_dbfDrop( LPRDDNODE pRDD, PHB_ITEM pItemTable, PHB_ITEM pItemIndex, ULONG ulConnect )
{
   ...
   if( hb_FileNetExists( ( BYTE * ) szFileName, ( BYTE * ) szFileName ) )
   {
      fResult = hb_fileNetDelete( ( BYTE * ) szFileName, 2 );
      if( fResult && fTable )
      {
         /*
         ...
         if( SELF_RDDINFO( pRDD, RDDI_MEMOEXT, ulConnect, pFileExt ) == HB_SUCCESS )
         {
            ...
            if( szExt[ 0 ] )
            {
               ...
               hb_fsFNameMerge( szFileName, pFileName );
               hb_fileNetDelete( ( BYTE * ) szFileName, 2 );
            }
         }
         /*
...
static HB_ERRCODE hb_dbfExists( LPRDDNODE pRDD, PHB_ITEM pItemTable, PHB_ITEM pItemIndex, ULONG ulConnect )
{
   ...
   return hb_FileNetExists( ( BYTE * ) szFileName, NULL ) ? HB_SUCCESS : HB_FAILURE;
}
...
HB_CALL_ON_STARTUP_END( _hb_dbfnet_rdd_init_ )
   ...
   static HB_$INITSYM hb_vm_auto_dbf1net__InitSymbols = dbf1net__InitSymbols;
   static HB_$INITSYM hb_vm_auto_dbf_rdd_init = _hb_dbfnet_rdd_init_;
   #pragma data_seg()
#endif
