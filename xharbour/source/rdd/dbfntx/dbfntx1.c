/*
 * $Id: dbfntx1.c,v 1.94 2002/01/25 18:36:19 alkresin Exp $
 */
...
static ERRCODE hb_ntxIndexCreate( LPNTXINDEX pIndex )
{
   ...
   if( ulRecCount )
   {
      sortInfo.sortBuffer = (BYTE*) hb_xgrab( ulRecCount * sortInfo.itemLength );
      if( !sortInfo.sortBuffer )
      {
         ...
         do
         {
            ...
            sortInfo.sortBuffer = (BYTE*) hb_xgrab(
                  (ulRecCount/nParts+1) * sortInfo.itemLength );
            if( sortInfo.sortBuffer )
               sortInfo.swappages = (BYTE*) hb_xgrab( nParts * sizeof( SWAPPAGE ) );
