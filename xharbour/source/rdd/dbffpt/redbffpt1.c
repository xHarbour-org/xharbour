   ...
   for( i = 0; i < pGCtable->usItems; i++ )
   {
      if( pGCtable->pGCitems[i].fChanged )
      {
             ...
             sizeof( FPTBLOCK ) )
         {
            ...
            if( hb_fileNetWriteAt( pArea->pMemoFile, ( BYTE * ) &fptBlock,
                                   sizeof( FPTBLOCK ),
...
static HB_ERRCODE hb_fptReadGCdata( FPTAREAP pArea, LPMEMOGCTABLE pGCtable )
{
   ...
   if( hb_fileNetReadAt( pArea->pMemoFile, ( BYTE * ) &pGCtable->fptHeader,
...
static HB_ERRCODE hb_fptWriteGCdata( FPTAREAP pArea, LPMEMOGCTABLE pGCtable )
{
   ...
   if( pGCtable->bChanged > 0 )
   {
      ...
      if( errCode == HB_SUCCESS )
      {
         ...
         if( hb_fileNetWriteAt( pArea->pMemoFile, ( BYTE * ) &pGCtable->fptHeader,
      ...
      if( ulBlock != 0 )
      {
                              ...
                              pArea->bMemoType == DB_MEMO_FPT ) )
         {
            ...
            if( pArea->bMemoType == DB_MEMO_DBT )
            {
               ...
               do
               {
                  ulLen = hb_fileNetReadAt( pArea->pMemoFile, pBlock, DBT_DEFBLOCKSIZE, fOffset );
                  fOffset += ulLen;
                  if( ulLen <= 0 )
                     break;
                  u = 0;
            ...
            else if( hb_fileNetReadAt( pArea->pMemoFile, ( BYTE * ) &fptBlock,
                                       ...
                                       HB_FOFFSET size )
{
   ...
   if( size )
   {
      ...
      do
      {
         ulRead = hb_fileNetReadAt( pSrc, pBuffer, ( ULONG )
   ...
   if( bWrite )
   {
      ...
      if( errCode == HB_SUCCESS && ulLen > 0 )
      {
         ...
         if( pFile )
         {
            ...
            do
            {
               ulRead = hb_fileNetReadLarge( pFile, bBuffer, HB_MIN( ulBufSize, ulLen - ulWritten ) );
               if( ulRead <= 0 )
                  errCode = EDBF_READ;
      ...
      if( errCode == HB_SUCCESS )
      {
         if( pArea->bMemoType == DB_MEMO_DBT )
         {
            hb_fileNetWriteAt( pArea->pMemoFile, ( BYTE * ) "\x1A\x1A", 2, fOffset );
         }
             ...
             ( ulLen + sizeof( FPTBLOCK ) ) % pArea->uiMemoBlockSize != 0 )
         {
            ULONG ulBlocks = ( ulLen + sizeof( FPTBLOCK ) + pArea->uiMemoBlockSize - 1 ) /
                              pArea->uiMemoBlockSize;
            hb_fileNetWriteAt( pArea->pMemoFile, ( BYTE * ) "\xAF", 1,
      ...
      else if( pField->uiLen >= 6 )
      {
         ...
         if( uiType <= HB_VF_CHAR ) /* 64000 max string size */
         {
            ...
            if( uiError == HB_SUCCESS )
            {
               ...
               if( pFile )
               {
                  if( hb_fileNetWrite( pFile, ( BYTE * ) pString, uiType ) != uiType )
                     uiError = EDBF_WRITE;
                  hb_xfree( pString );
               }
               else
...
*/
   ...
   if( hb_fileNetWriteAt( pArea->pMemoFile, ( BYTE * ) &fptHeader, ulSize, 0 ) != ulSize )
      return HB_FAILURE;
   ...
   if( ulLen > ulSize )
   {
      ...
      do
      {
         ...
         if( hb_fileNetWriteAt( pArea->pMemoFile, ( BYTE * ) &fptHeader,
...
static HB_ERRCODE hb_fptOpenMemFile( FPTAREAP pArea, LPDBOPENINFO pOpenInfo )
{
   ...
   else
   {
      ...
      if( hb_fptFileLockSh( pArea, TRUE ) )
      {
         if( hb_fileNetReadAt( pArea->pMemoFile, ( BYTE * ) &fptHeader,
                            sizeof( FPTHEADER ), 0 ) >= 512 )
         {
            pArea->uiMemoVersion = DB_MEMOVER_STD;
            if( pArea->bMemoType == DB_MEMO_SMT )
   ...
   else if( pArea->fHasMemo && pArea->pMemoFile && pArea->pDataFile )
   {
      ...
      if( errCode == HB_SUCCESS && ulRecords )
      {
         ...
         if( pArea->pMemoTmpFile )
         {
            ...
            hb_fileNetDelete( ( BYTE * ) szFile, uiRemote );
...
HB_CALL_ON_STARTUP_END( _hb_redbffpt_rdd_init_ )
   ...
   static HB_$INITSYM hb_vm_auto_redbffpt1__InitSymbols = redbffpt1__InitSymbols;
   static HB_$INITSYM hb_vm_auto_redbffpt_rdd_init = _hb_redbffpt_rdd_init_;
   #pragma data_seg()
#endif
