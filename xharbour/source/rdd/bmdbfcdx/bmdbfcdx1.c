...
static ULONG hb_cdxRecNoInFilterArray( CDXAREAP pArea );
...
HB_FUNC( BM_DBSETFILTERARRAY )
{
    ...
    if( pArea )
    {
       if( pArray )
       {
            ...
            {
                pTag->logKeyCount = hb_arrayLen( pArray );
                pTag->curKeyState |= CDX_CURKEY_LOGCNT;
            }
       }
       else
