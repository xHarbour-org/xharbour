...
static USHORT   s_uiStatics = 0;    /* Number of statics added after processing hb_vmStatics() */
   ...
   while( (BOOL) TRUE )
   {
      ...
      switch( pCode[ w ] )
      {
         ...
         case HB_P_BASELINE:
            ...
            hb_stackBaseItem()->item.asSymbol.lineno = (USHORT) s_iBaseLine;
         ...
         case HB_P_LINE:
            HB_TRACE(HB_TR_INFO, ("Opcode: HB_P_LINE: %s (%i)",
                     ...
                     hb_stackBaseItem()->item.asSymbol.lineno));
         ...
         case HB_P_LINEOFFSET:
            ...
            hb_stackBaseItem()->item.asSymbol.lineno = (USHORT) ( s_iBaseLine + (BYTE) pCode[ w + 1 ] );
            if( s_bDebugging )
            {
               hb_vmDebuggerShowLine( hb_stackBaseItem()->item.asSymbol.lineno );
            }
...
*/
         ...
         case HB_P_PUSHGLOBALREF:
            HB_TRACE( HB_TR_DEBUG, ("HB_P_PUSHGLOBALREF") );
            {
               ...
               pTop->item.asRefer.BasePtr.itemsbasePtr = HB_GETGLOBALSPTR();
...
static void hb_vmBitAnd( void )
{
   ...
   else if( HB_IS_STRING( pItem1 ) &&  HB_IS_STRING( pItem2 ) )
   {
      ...
      if( ulLen1 && ulLen2 )
      {
         ...
         register ULONG ulPos1;
		 register ULONG ulPos2;
...
static HARBOUR hb_vmDoBlock( void )
{
   ...
   uiLine = pBaseSym->item.asSymbol.lineno;

   // Change the line number to line where block was defined.
   pBaseSym->item.asSymbol.lineno = pBlock->item.asBlock.value->lineno;
   ...
   // Restore line numer.
   pBaseSym->item.asSymbol.lineno = uiLine;
...
static void hb_vmDebugEntry( int nMode, int nLine, char *szName, int nIndex, int nFrame )
{
   ...
   switch( nMode )
   {
      ...
      case HB_DBG_STATICNAME:
         ...
         hb_vmPushLong( HB_VM_STACK.iStatics );   /* current static frame */
...
static void hb_vmStaticName( USHORT uiStatic, char * szStaticName ) /* statics vars information for the debugger */
{
   ...
   {
      s_bDebugging = FALSE;
      s_pFunDbgEntry( HB_DBG_STATICNAME, 0, szStaticName, uiStatic, HB_VM_STACK.iStatics );
      s_bDebugging = TRUE;
   }
}
...
HB_EXPORT void hb_vmPushInteger( int iNumber )
{
   ...
   ( * HB_VM_STACK.pPos )->item.asInteger.length = (UINT) HB_INT_LENGTH( iNumber );
   hb_stackPush();
}
...
static void hb_vmPushIntegerConst( int iNumber )
{
   ...
   ( * HB_VM_STACK.pPos )->type = HB_IT_INTEGER;
   ( * HB_VM_STACK.pPos )->item.asInteger.value = iNumber;
   ( * HB_VM_STACK.pPos )->item.asInteger.length = (UINT) hb_vmCalcIntWidth( iNumber );
   hb_stackPush();
}
...
static void hb_vmPushLongConst( LONG lNumber )
{
   ...
   ( * HB_VM_STACK.pPos )->item.asInteger.length = (UINT) hb_vmCalcIntWidth( lNumber );
#else
   ( * HB_VM_STACK.pPos )->type = HB_IT_LONG;
...
HB_EXPORT void hb_vmPushLong( LONG lNumber )
{
   ...
   ( * HB_VM_STACK.pPos )->type = HB_IT_INTEGER;
   ( * HB_VM_STACK.pPos )->item.asInteger.value = ( int ) lNumber;
   ( * HB_VM_STACK.pPos )->item.asInteger.length = (UINT) HB_INT_LENGTH( lNumber );
#else
   ( * HB_VM_STACK.pPos )->type = HB_IT_LONG;
...
static void hb_vmPushHBLong( HB_LONG lNumber )
{
   ...
   ( * HB_VM_STACK.pPos )->type = HB_IT_LONG;
   ( * HB_VM_STACK.pPos )->item.asLong.value = lNumber;
   ( * HB_VM_STACK.pPos )->item.asLong.length = (UINT) HB_LONG_LENGTH( lNumber );
...
static void hb_vmPushLongLongConst( LONGLONG llNumber )
{
   ...
   ( * HB_VM_STACK.pPos )->item.asLong.length = (UINT) hb_vmCalcIntWidth( llNumber );
...
HB_EXPORT void hb_vmPushDouble( double dNumber, int iDec )
{
   ...
   ( * HB_VM_STACK.pPos )->item.asDouble.length = (UINT) HB_DBL_LENGTH( dNumber );
   ( * HB_VM_STACK.pPos )->item.asDouble.decimal = (UINT) iDec;
   if( iDec == HB_DEFAULT_DECIMALS )
   {
     ( * HB_VM_STACK.pPos )->item.asDouble.decimal = (UINT) hb_set.HB_SET_DECIMALS;
   }
...
static void hb_vmPushDoubleConst( double dNumber, int iWidth, int iDec )
{
   ...
   ( * HB_VM_STACK.pPos )->item.asDouble.value = dNumber;
   ( * HB_VM_STACK.pPos )->item.asDouble.decimal = (UINT) iDec;
   ( * HB_VM_STACK.pPos )->item.asDouble.length = (UINT) iWidth;
   if( iDec == HB_DEFAULT_DECIMALS )
   {
     ( * HB_VM_STACK.pPos )->item.asDouble.decimal = (UINT) hb_set.HB_SET_DECIMALS;
   }
...
static void hb_vmPushMacroBlock( BYTE * pCode )
{
   ...
   pTop->item.asBlock.value->uLen = (USHORT) ( HB_PCODE_MKUSHORT( &( pCode[ 1 ] ) ) - 5 );
}
...
HB_FUNC( HB_RESTOREBLOCK )
{
   ...
   if ( pBlockAsArray && ( hb_arrayLen( pBlockAsArray ) == 4 || hb_arrayLen( pBlockAsArray ) == 7 ) )
   {
      ...
      hb_arrayGet( pBlockAsArray, 7, &iStatics );
      ...
      if( HB_IS_STRING( &ModuleName ) && HB_IS_STRING( &PCode ) && HB_IS_INTEGER( &ParamCount ) && HB_IS_INTEGER( &ClassH ) )
      {
         ...
         if( HB_IS_STRING( &ProcName ) && HB_IS_INTEGER( &ProcLine ) && HB_IS_INTEGER( &iStatics ) )
         {
            ...
            Block.item.asBlock.statics       = iStatics.item.asInteger.value;
         }
...
HB_EXPORT void hb_xvmLineOffset( BYTE bLine )
{
   ...
   hb_stackBaseItem()->item.asSymbol.lineno = uiLineno;
