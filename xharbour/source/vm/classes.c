...
static HARBOUR hb___msgSuper( void )
{
   ...
   pReturn->item.asArray.value->uiPrevCls  = hb_objClassH( pObject ); /* backup of actual handel */
   pReturn->item.asArray.value->uiClass    = (HB_VM_STACK.pMethod)->uiSprClass;     /* superclass handel casting */
}
   ...
   if( uiClass && uiClass <= s_uiClasses )
   {
      ...
      if( pClass->pDestructor )
      {
         if( pClass->uiScope & HB_OO_CLS_DESTRUC_SYMB )
         {
            ...
            {
               if( HB_IS_SYMBOL( hb_stackBaseItem() ) && hb_stackBaseItem()->item.asSymbol.pCargo->uiSuperClass == 0 && strcmp( hb_stackBaseItem()->item.asSymbol.value->szName, "__CLSINSTSUPER" ) )
               {
