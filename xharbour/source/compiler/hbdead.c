...
static HB_CODETRACE_FUNC( hb_p_localnearsetstr )
{
   BYTE * pAddr = &pFunc->pCode[ lPCodePos + 2 ];
   ULONG ulSize = 4 + HB_PCODE_MKUSHORT( pAddr );

   hb_compCodeTraceMark( cargo, lPCodePos, ulSize );
   return hb_compCodeTraceNextPos( cargo, lPCodePos + ulSize );
}

static HB_CODETRACE_FUNC( hb_p_pushstrhidden )
{
   BYTE * pAddr = &pFunc->pCode[ lPCodePos + 4 ];
   ULONG ulSize = 6 + HB_PCODE_MKUSHORT( pAddr );

   hb_compCodeTraceMark( cargo, lPCodePos, ulSize );
   return hb_compCodeTraceNextPos( cargo, lPCodePos + ulSize );
}

static HB_CODETRACE_FUNC( hb_p_localnearsetstrhidden )
{
   BYTE * pAddr = &pFunc->pCode[ lPCodePos + 5 ];
   ULONG ulSize = 7 + HB_PCODE_MKUSHORT( pAddr );
...
static PHB_CODETRACE_FUNC s_codeTraceFuncTable[ HB_P_LAST_PCODE ] =
{
   ...
   hb_p_default,               /* HB_IVARREF,                */
   hb_p_default,               /* HB_CLASSSETMODULE,         */
