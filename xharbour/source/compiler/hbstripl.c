/*
 * $Id: hbstripl.c,v 1.1 2006/02/04 16:16:45 druzus Exp $
 */
...
static HB_STRIP_FUNC( hb_p_localnearsetstr )
{
   HB_SYMBOL_UNUSED( cargo );
   return 4 + HB_PCODE_MKUSHORT( &( pFunc->pCode[ lPCodePos + 2 ] ) );
}

static HB_STRIP_FUNC( hb_p_pushstrhidden )
{
   HB_SYMBOL_UNUSED( cargo );
   return 6 + HB_PCODE_MKUSHORT( &( pFunc->pCode[ lPCodePos + 4 ] ) );
}

static HB_STRIP_FUNC( hb_p_localnearsetstrhidden )
{
   HB_SYMBOL_UNUSED( cargo );
...
void hb_compStripFuncLines( PFUNCTION pFunc )
{
   ...
   hb_compPCodeEval( pFunc, ( HB_PCODE_FUNC_PTR * ) s_stripLines_table, NULL, FALSE );
}
