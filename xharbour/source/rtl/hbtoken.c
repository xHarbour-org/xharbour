...
static ULONG hb_tokenCount( const char * szLine, ULONG nLen,
                              const char * szDelim, ULONG nDelim,
                              int iFlags )
{
   ULONG ul = 0, nTokens = 1;
   char cQuote = 0;
...
static const char * hb_tokenGet( const char * szLine, ULONG nLen,
                                 const char * szDelim, ULONG nDelim,
                                 int iFlags, ULONG nToken, ULONG * pnLen )
{
   ULONG ul, nStart;
   char cQuote = 0;
...
static PHB_ITEM hb_tokenArray( const char * szLine, ULONG nLen,
                               const char * szDelim, ULONG nDelim,
                               int iFlags )
{
   ULONG nTokens = hb_tokenCount( szLine, nLen, szDelim, nDelim, iFlags );
   PHB_ITEM pArray = hb_itemArrayNew( nTokens );

   if( nTokens )
   {
      ULONG ul, nStart, nToken;
      char cQuote = 0;
...
static BOOL hb_tokenParam( int iParam, ULONG nSkip,
                              const char ** pszLine, ULONG * pnLen,
                              const char ** pszDelim, ULONG * pnDelim,
                              int * piFlags )
{
   const char * szLine = hb_parc( 1 ), * szDelim = NULL;
   ULONG nLen = hb_parclen( 1 ), nDelim = 0;
   int iFlags = 0;
...
HB_FUNC( HB_TOKENCOUNT )
{
   ...
   if( hb_tokenParam( 2, 0, &szLine, &nLen, &szDelim, &nDelim, &iFlags ) )
      hb_retnl( hb_tokenCount( szLine, nLen, szDelim, nDelim, iFlags ) );
   else
      hb_retnl( 0 );
}
...
HB_FUNC( HB_TOKENPTR )
{
   ...
   ULONG nLen, nDelim, nSkip, nToken;
   int iFlags;
