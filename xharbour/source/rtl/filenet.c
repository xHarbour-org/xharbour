                            ...
                            HB_FOFFSET ulStart, HB_FOFFSET ulLen )
{
   ...
   uiPos = hb_fileNetFindOffset( pFile, ulStart );
   fLJoin = fRJoin = FALSE;
 ...
 */
 ...
 static char * hb_strToken( char * szText, ULONG ulText, ULONG ulIndex, ULONG * pulLen )
{
   ULONG ulStart;
   ULONG ulEnd = 0;
   ULONG ulCounter = 0;
...
static int hb_NetSingleSendRecv( HB_SOCKET_T hCurSocket, char * sData, ULONG ulLen, int iErr )
{
   long int lRet;
...
static int hb_NetSendSingleRecv( HB_SOCKET_T hCurSocket, char * sData, ULONG ulLen, int iErr )
{
   long int lRet;
...
static long int hb_NetDataSingleSendSingleRecv( HB_SOCKET_T hCurSocket, char * szData, ULONG ulLen )
{
#if defined(_DBG_)
   OutputDebugString( "---------------------1" );
   OutputDebugString( szData + HB_LENGTH_ACK );
...
static int hb_NetSingleSendSingleRecv( HB_SOCKET_T hCurSocket, char * sData, ULONG ulLen, int iErr )
{
   long int lRet;

   lRet = hb_NetDataSingleSendSingleRecv( hCurSocket, sData, ulLen );
...
USHORT hb_fileNetCurDirBuffEx( USHORT uiDrive, char * pbyBuffer, ULONG ulLen )
{
   if( hSocket )
   {
      ...
      nSend = hb_snprintf( szData + HB_LENGTH_ACK, sizeof( szData ),"W|%hu|%lu|\r\n", uiDrive, ulLen );
                            ...
                            PHB_ITEM pError, BOOL fBufferLock )
{
   ...
   else
   {
      if( hSocket )
      {
         ...
         int nSend = hb_snprintf( szData + HB_LENGTH_ACK, sizeof( szData ), "A|%s|%s|%hu|%s|\r\n", pszFile, ( pDefExt ? pDefExt : ( char * ) "" ),
...
void hb_fileNetClose( PHB_FILE pFile )
{
   ...
   if( hFile != FS_ERROR )
   {
      ...
      if( hCurSocket )
      {
         nSend = hb_snprintf( szData + HB_LENGTH_ACK, sizeof( szData ), "B|%p|\r\n", ( void * ) hFile );
         HB_PUT_BE_UINT32( szData, nSend );
...
BOOL hb_fileNetLock( PHB_FILE pFile, HB_FOFFSET ulStart, HB_FOFFSET ulLen, int iType )
{
   ...
   if( ( iType & FL_MASK ) == FL_UNLOCK )
   {
      ...
      if( fLockFS )
      {
         ...
         if( pFile->hSocket )
         {
            nSend = hb_snprintf( szData + HB_LENGTH_ACK, sizeof( szData ), "C|%p|%" PFHL "i|%" PFHL "i|%hu|\r\n", ( void * ) pFile->hFile, ulStart, ulLen, ( USHORT ) iType );
            HB_PUT_BE_UINT32( szData, nSend );
...
ULONG hb_fileNetReadAt( PHB_FILE pFile, void * pBuffer, ULONG ulSize, HB_FOFFSET llOffset )
{
   ...
   if( pFile->hSocket )
   {
      ...
      nSend = hb_snprintf( szData + HB_LENGTH_ACK, sizeof( szData ), "D|%p|%lu|%" PFHL "i|\r\n", ( void * ) pFile->hFile, ulSize, llOffset );
      HB_PUT_BE_UINT32( szData, nSend );
...
ULONG hb_fileNetReadLarge( PHB_FILE pFile, void * pBuffer, ULONG ulSize )
{
   ...
   if( pFile->hSocket )
   {
      ...
      nSend = hb_snprintf( szData + HB_LENGTH_ACK, sizeof( szData ), "K|%p|%lu|\r\n", ( void * ) pFile->hFile, ulSize );
      HB_PUT_BE_UINT32( szData, nSend );
...
ULONG hb_fileNetWriteAt( PHB_FILE pFile, const void * buffer, ULONG ulSize, HB_FOFFSET llOffset )
{
   ...
   if( pFile->hSocket )
   {
      ...
      ulLen = hb_snprintf( szData + HB_LENGTH_ACK, 40 + ulSize, "E|%p|%lu|%" PFHL "i|", ( void * ) pFile->hFile, ulSize, llOffset );
...
ULONG hb_fileNetWriteLarge( PHB_FILE pFile, const void * pBuffer, ULONG ulSize )
{
   ...
   if( pFile->hSocket )
   {
      ...
      ulLen = hb_snprintf( szData + HB_LENGTH_ACK, 36 + ulSize + HB_LENGTH_ACK, "J|%p|%lu|", ( void * ) pFile->hFile, ulSize );
      ptr = szData + HB_LENGTH_ACK + ulLen;
...
USHORT hb_fileNetWrite( PHB_FILE pFile, const char * pBuffer, USHORT uiCount )
{
   ...
   if( pFile->hSocket )
   {
      ...
      ulLen = hb_snprintf( szData + HB_LENGTH_ACK, 36 + uiCount + HB_LENGTH_ACK, "O|%p|%hu|", ( void * ) pFile->hFile, uiCount );
      ptr = szData + HB_LENGTH_ACK + ulLen;
...
BOOL hb_fileNetTruncAt( PHB_FILE pFile, HB_FOFFSET llOffset )
{
   if( pFile->hSocket )
   {
      ...
      nSend = hb_snprintf( szData + HB_LENGTH_ACK, sizeof(szData), "F|%p|%" PFHL "i|\r\n", ( void * ) pFile->hFile, llOffset );
      HB_PUT_BE_UINT32( szData, nSend );
...
HB_FOFFSET hb_fileNetSeekLarge( PHB_FILE pFile, HB_FOFFSET llOffset, USHORT uiFlags )
{
   ...
   if( pFile->hSocket )
   {
      ...
      nSend = hb_snprintf( szData + HB_LENGTH_ACK, sizeof(szData), "G|%p|%" PFHL "i|%hu|\r\n", ( void * ) pFile->hFile, llOffset, uiFlags );
      HB_PUT_BE_UINT32( szData, nSend );
...
void hb_fileNetCommit( PHB_FILE pFile )
{
   if( pFile->hSocket )
   {
      ...
      nSend = hb_snprintf( szData + HB_LENGTH_ACK, sizeof(szData), "H|%p|\r\n", ( void * ) pFile->hFile );
      HB_PUT_BE_UINT32( szData, nSend );
...
BOOL hb_fileNetDelete( const char * pFileName, USHORT uiRemote )
{
   ...
   else
   {
      ...
      nSend = hb_snprintf( szData + HB_LENGTH_ACK, sizeof(szData), "I|%s|\r\n", pFileName );
      HB_PUT_BE_UINT32( szData, nSend );
...
BOOL hb_fileNetRename( const char * pOldName, const char * pNewName )
{
   ...
   nSend = hb_snprintf( szData + HB_LENGTH_ACK, sizeof(szData), "V|%s|%s|\r\n", pOldName, pNewName );
   HB_PUT_BE_UINT32( szData, nSend );
...
static BOOL hb_NetFileExists( const char * pszFileName )
{
   ...
   {
      char szData[HB_PATH_MAX + 5 + HB_LENGTH_ACK];
      int nSend = hb_snprintf( szData + HB_LENGTH_ACK, sizeof(szData), "M|%s|\r\n", pszFileName );
      HB_PUT_BE_UINT32( szData, nSend );
...
BOOL hb_FileNetFile( char * pFileName )
{
   ...
   {
      char szData[HB_PATH_MAX + 5 + HB_LENGTH_ACK];
      int nSend = hb_snprintf( szData + HB_LENGTH_ACK, sizeof(szData), "N|%s|\r\n", pFileName );
      HB_PUT_BE_UINT32( szData, nSend );
...
PHB_NETFFIND hb_FileNetFindFirst( const char * pszFileName, ULONG ulAttr )
{
   ...
   nSend = hb_snprintf( szData + HB_LENGTH_ACK, sizeof(szData), "P|%s|%lu|\r\n", pszFileName, ulAttr );
   HB_PUT_BE_UINT32( szData, nSend );
...
BOOL hb_FileNetFindNext( PHB_NETFFIND pffind )
{
   ...
   nSend = hb_snprintf( szData + HB_LENGTH_ACK, sizeof(szData), "Q|%p|\r\n", pffind->pNetffind );
   HB_PUT_BE_UINT32( szData, nSend );
...
void hb_FileNetFindClose( PHB_NETFFIND pffind )
{
   ...
   nSend = hb_snprintf( szData + HB_LENGTH_ACK, sizeof(szData), "R|%p|\r\n", pffind->pNetffind );
   HB_PUT_BE_UINT32( szData, nSend );
...
ULONG hb_fileNetGetFileAttributes( char * pFilename )
{
   ...
   nSend = hb_snprintf( szData + HB_LENGTH_ACK, sizeof(szData), "S|%s|\r\n", pFilename );
   HB_PUT_BE_UINT32( szData, nSend );
...
BOOL hb_fileNetMkDir( char * pPath )
{
   ...
   nSend = hb_snprintf( szData + HB_LENGTH_ACK, sizeof(szData), "T|%s|\r\n", pPath );
   HB_PUT_BE_UINT32( szData, nSend );
...
BOOL hb_fileNetRmDir( char * pPath )
{
   ...
   nSend = hb_snprintf( szData + HB_LENGTH_ACK, sizeof(szData), "U|%s|\r\n", pPath );
   HB_PUT_BE_UINT32( szData, nSend );
