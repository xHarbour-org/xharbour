...
if [ -z "$HB_ARCHITECTURE" ]; then export HB_ARCHITECTURE=darwin; fi
...
if [ -z "$HB_BIN_INSTALL" ]; then export HB_BIN_INSTALL=/usr/local/bin/; fi    
if [ -z "$HB_LIB_INSTALL" ]; then export HB_LIB_INSTALL=/usr/local/lib/; fi    
if [ -z "$HB_INC_INSTALL" ]; then export HB_INC_INSTALL=/usr/local/include/; fi
