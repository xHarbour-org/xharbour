/*
 * $Id: hbcu.c,v 1.1 2004/01/14 13:59:52 andijahja Exp $
 */
	...
	while (i<srclen) {
		if (i%UE_STR_LEN==0) {
			if (dststr) {
				...
				if ((srclen-i)>UE_STR_LEN) dststr[dstlen++]=int_uubyte(UE_STR_LEN);
				else dststr[dstlen++]=int_uubyte((BYTE)(srclen-i));
			}
			else dstlen+=1+(i?OS_EOL_LEN:0);
		}
		if (dststr) dststr[dstlen++]=int_uubyte((srcstr[i]&0xFC)>>2);
		else dstlen++;
		if (++i==srclen) {
			if (dststr) {
		...
		if (dststr) dststr[dstlen++]=int_uubyte(((srcstr[i-1]&0x03)<<4)|((srcstr[i]&0xF0)>>4));
		else dstlen++;
		if (++i==srclen) {
			if (dststr) {
