...
/* The aim of this test is to check the same RDD functions and statements against a dbf file 
   and the same dbf imported into a MySQL database.
...
procedure main()
   ...
   nConn := dbmysqlconnection( "<host>;<database>;<user>;<password>;<port>" )

   /* if you want to update and insert data you need at least a primary key */

   use "select <fields,...> from <table> order by <same order as dbf>" alias <table> via "mysqlrdd" connection nConn 
