/*
 * $Id: dir.c,v 1.2 2002/01/03 03:53:44 ronpinkas Exp $
 */
   ...
   while( ( entry = readdir( dir ) ) != NULL )
   {
      ...
      if( hb_strMatchRegExp( fname, pfname ) && hb_strMatchRegExp( fext, pfext ) )
      {
                ...
                ( ( ushbMask & FA_DIREC  ) == 0 && ( attrib & FA_DIREC  ) > 0 ) ) )
         {
            ...
            hb_arraySet( pSubarray, F_NAME, pFilename );
            hb_arraySet( pSubarray, F_SIZE, pSize );
            hb_arraySet( pSubarray, F_DATE, pDate );
            hb_arraySet( pSubarray, F_TIME, pTime );
            hb_arraySet( pSubarray, F_ATTR, pAttr );
