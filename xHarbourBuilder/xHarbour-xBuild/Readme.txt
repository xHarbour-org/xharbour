...
(but can also be used with plain C projects) and able to build:
...
>From any combination of:
  ...
  LIB Files
  RES Files
  XBP Files (project files generated by xBuild)
...
-----------------
  ...
  - May combine command line arguments as well as Project Files and
    Wizard settings.
  ...
  - Full support for long file names, including Spaces (except in target 
    executables).

Syntax:
--------

xBuild [<target.exe|lib|dll>|<MainSource>] [<MoreSourceMask>] [switches]
...
Switches:
---------
 -All (Force clean build.)                                        
 -CON (direct error log to console instead of NotePad.)           
 -C<Bcc|Gcc|Mingw|Vc|xCC>=<RootFolder>[,C_Flags]                  
 ...
 -T<TargetFolder> (Will override target location.)                
...
------------------
   ...
   xbuild mymain myotherprg mycfile.c
...
As always it will also create mymain.exe.xbp in the current folder.
...
Assume you have an application which must link against a few Libraries which
you must build as well.

Well now you may easily create one project for each of the Libs as follows:
...
If you are as Lazy as I am, you would use the Wildcard feature just to gather
...
If you have any comments or suggestions, please mail them
to xBuild@xHarbour.com
...
Latest revised on January 5th, 2004