...
/* get a floating point value for __scanf */
int __getflt(__scninfo *px)
{
        ...
        else
        {
            ...
            *p++ = 'n', ch = GETN(px);
            ...
            else
            {
                ...
                if ((ch = GETN(px)) != 'n' && ch != 'N' || (ch = GETN(px)) != 'i' && ch != 'I' || (ch = GETN(px)) != 't' && ch != 'T' || (ch = GETN(px)) != 'y' && ch != 'Y')
                    UNGETN(px, ch);
                else
                {
    ...
    if (dlen > 0)
    {
        ...
        for (nsig = 0; ch != EOF && memchr(&digits[0], ch, dlen); ch = GETN(px), seen = 1)
        {
        ...
        if (seen && (dlen == 10 && (ch == 'e' || ch == 'E') || dlen != 10 && (ch == 'p' || ch == 'P')))
        {
            /* parse exponent */
            *p++ = ch, ch = GETN(px);
            if (ch == '+' || ch == '-')
    ...
    *p = '\0';
    if (!px->noconv)
    {
        ...
        if (dlen <= 10)
        {
            ldval = __stold(ac, 0, pten);
        }
        else
        {
            /* convert fraction and scale by hex exponent */
            ldval = __stold(ac, 0, 0);
