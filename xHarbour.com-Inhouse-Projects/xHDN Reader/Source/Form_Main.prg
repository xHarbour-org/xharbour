...
//-- PROGRAM START -----------------------------------------------------------------------------------//
METHOD Form_Main_OnShowWindow( Sender ) CLASS Form_Main
...
//-- LISTBOX SELECTION -------------------------------------------------------------------------------//
METHOD ListBox_SearchResults_OnLButtonUp( Sender ) CLASS Form_Main
   IF Sender:GetCurSel() >= 0 .AND. Sender:GetCurSel() < Sender:GetCount()
      ShowInfo(aDBKeys[Sender:GetCurSel() + 1], ::Panel_DocContent)
   ELSE
...
//-- LISTBOX SELECTION -------------------------------------------------------------------------------//
METHOD ListBox_SearchResults_OnKeyUp( Sender ) CLASS Form_Main
...
//-- ADVANCEC OPTIONS --------------------------------------------------------------------------------//
METHOD Button_AdvancedOptions_OnClick( Sender ) CLASS Form_Main
   IF ::Panel_Search:Height == 93
      ::Panel_Search:Height := 193
      ::Panel_SearchResults:Height := ::Panel_DocContent:Height - ::Panel_SearchResults:Top - ::Panel_DocContent:Top
      ::Panel_SearchResults:Top := ::Panel_SearchResults:Top + 100
   ELSE
      ::Panel_Search:Height := 93
      ::Panel_SearchResults:Height := ::Panel_DocContent:Height - ::Panel_SearchResults:Top - ::Panel_DocContent:Top
      ::Panel_SearchResults:Top := 124
   END
...
METHOD Button_Search_OnClick( Sender ) CLASS Form_Main
   ...
   IF LEN(::Edit_Search:Caption) > 0
      FOR i := 1 TO LEN(aDBKeys)
         IF UPPER(ALLTRIM(::Edit_Search:Caption)) $ UPPER(ALLTRIM(aDBKeys[i][4]))
...
//----------------------------------------------------------------------------------------------------//
METHOD Edit_Search_OnKeyUp( Sender ) CLASS Form_Main
   LOCAL i
