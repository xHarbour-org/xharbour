...
METHOD ConnectRaw( cDSN, cUser, cPassword, nVersion, cOwner, nSizeMaxBuff, lTrace,;
   ...
   If (!::lQueryOnly) .and. cSystemVers < MINIMAL_MYSQL_SUPPORTED
      SR_MsgLogFile( "Connection Error: MySQL version not supported : " + cSystemVers + " / minimun is " + MINIMAL_MYSQL_SUPPORTED )
      ::End()
      ::nSystemID := 0
      ::nRetCode  := -1
      Return Self
