...
#xtranslate ISSTRUCT(<x>) => ( Valtype(<x>)  =="O" .AND. Left( <x>:ClassName, 8 ) == "_STRUCT_" )
...
*------------------------------------------------------------------------------*
   ...
   IF ValType( dbg_array ) $ 'AOS'
      IF ISSTRUCT( dbg_array)
         OutputDebugString( description +" (structure) :"+CRLF )
         dbg_array:debug()
      ELSE
         a_len = Len( dbg_array )
...
*------------------------------------------------------------------------------*
   ...
   CASE data_type == "N"
      DQOUT( "   Numeric:", heading, " => ", value )
   CASE data_type == "P"
      OutputDebugString( "  Pointer vv" +CRLF) // arrows don't show in Windows
   CASE data_type == "U"
