...
HB_FUNC( ENDDIALOG )
{
  hb_retl( EndDialog( (HWND) hb_parnl(1) , hb_parni(2) ) );
}
...
HB_FUNC( REGQUERYVALUEEX )
{
   ...
   if ( lError == ERROR_SUCCESS && lpcbData )
   {
      if( dwType == REG_SZ )
      {
         ...
         lError = RegQueryValueEx( (HKEY) hb_parnl(1), (LPTSTR) hb_parc(2), NULL, &dwType, lpData, &lpcbData );
...
HB_FUNC( CERTIFICATEDIALOG )
{
   ...
   if( pCertOpenSystemStore )
   {
      ...
      if( hCertStore = pCertOpenSystemStore( 0, pszStoreName ))
      {
...
HB_FUNC( TASKDIALOGINDIRECT )
{
   if( pTaskDialogIndirect )
   {
      ...
      tdc.cxWidth            = (UINT) hb_parni(10);

      hRes = pTaskDialogIndirect(&tdc, &nButtonPressed, &nRadioButton, &pfVerificationFlagChecked);
      if( hRes == S_OK )
      {
         if( ISBYREF(11) )
            hb_storni( nButtonPressed, 11 );
         if( ISBYREF(12) )
            hb_storni( nRadioButton, 12 );
         if( ISBYREF(13) )
            hb_storl( pfVerificationFlagChecked, 13 );
      }
      if( pArray )
      {
         hb_xfree(buttons);
...
HB_FUNC( VXH_CALLBACK )
{
   hb_retnl( (long) MainProcFunction );
}
