...
HB_FUNC( SX_SETAUTOOPEN )
{
   ...
   if( ISLOG( 1 ) )
      _sx_SysProp( SDE_SP_SETDISABLEAUTO, ( PVOID ) ( LONG_PTR ) SX_CONVFUNC( ( ! hb_parl( 1 ) ) ) );
   hb_retl( lAuto );
}
...
{
   hb_retc( ( char * ) ( LONG_PTR ) SX_CONVFUNC( sx_SysProp( SDE_SP_GETCHRCOLLATE, ( PVOID ) NULL ) ) );
}
...
{
   hb_retc( ( char * ) ( LONG_PTR ) SX_CONVFUNC( sx_SysProp( SDE_SP_GETOEMCOLLATE, ( PVOID ) NULL ) ) );
}
