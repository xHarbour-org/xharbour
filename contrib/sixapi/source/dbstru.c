...
BOOL _sx_CopyStructure( PBYTE cpFileName, PHB_ITEM paFields )
{
   ...
   * cFieldToCopy;
   ...
   if( uilenpArray )
   {
      ...
      {
         cpFieldName = ( char * ) ( LONG_PTR ) SX_CONVFUNC( sx_FieldName( ( WORD ) ( i + 1 ) ) );
         ...
         for( ui = 0; ui < uilenpArray; ui++ )
         {
            ...
            if( strcmp( cpFieldName, cFieldToCopy ) == 0 )
            {
               ...
               pTemp   = hb_itemPutC( NULL, ( char * ) ( LONG_PTR ) SX_CONVFUNC( sx_FieldType( ( PBYTE ) cpFieldName ) ) );
...
PHB_ITEM _sx_DbStruct( void )
{
   ...
   for( uiCount = 1; uiCount <= uiFields; uiCount++ )
   {
      int   iWidth;
      int   iDec;
      PBYTE cFieldName = ( PBYTE ) ( LONG_PTR ) SX_CONVFUNC( sx_FieldName( ( WORD ) uiCount ) );
      ...
      hb_arraySet( pItem, 2, hb_itemPutC( pData, ( char * ) ( LONG_PTR ) SX_CONVFUNC( sx_FieldType( cFieldName ) ) ) );
...
PHB_ITEM _sx_FieldNames( void )
{
   ...
   for( uiCount = 1; uiCount <= uiFields; uiCount++ )
   {
      cFieldName = ( char * ) ( LONG_PTR ) SX_CONVFUNC( sx_FieldName( uiCount ) );
