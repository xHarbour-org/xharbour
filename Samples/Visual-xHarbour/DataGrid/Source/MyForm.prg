...
METHOD MyForm_OnLoad( Sender ) CLASS MyForm
   ...
   local nWidth:=GetSystemMetrics( SM_CXSCREEN )  // current screen width
   local nHeight:=GetSystemMetrics( SM_CYSCREEN ) // and height
   ::Width:=Int( nWidth*3/4 )   // setting application window width
   ::Height:=Int( nHeight*3/4 ) // and height
